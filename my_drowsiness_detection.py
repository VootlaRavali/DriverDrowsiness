...
mixer.init()
alarm_sound = mixer.Sound('C:\\Users\\raval\\OneDrive\\Desktop\\Ravlyy\\drivrDrows\\alarm.wav')

# this xml files are used to detect face , left eye , and right eye of a person.
face_detection = cv2.CascadeClassifier('C:\\Users\\raval\\OneDrive\\Desktop\\Ravlyy\\drivrDrows\haar cascade files\haarcascade_frontalface_alt.xml')
left_eye_detection = cv2.CascadeClassifier('C:\\Users\\raval\OneDrive\\Desktop\\Ravlyy\\drivrDrows\\haar cascade files\\haarcascade_lefteye_2splits.xml')
right_eye_detection = cv2.CascadeClassifier('C:\\Users\\raval\\OneDrive\\Desktop\\Ravlyy\\drivrDrows\\haar cascade files\\haarcascade_righteye_2splits.xml')

labels =['Close','Open']

# load the model, that we have created
model = load_model('C:\\Users\\raval\\OneDrive\\Desktop\\Ravlyy\\drivrDrows\\models\\custmodel.h5')
...
while(True):
    ...
    for (x,y,w,h) in right_eye:
        ...
        #right_eye_pred = model.predict_classes(right_one)   #classes
        predictions = model.predict(right_one)
        right_eye_pred = np.argmax(predictions, axis=1)
    ...
    for (x,y,w,h) in left_eye:
        ...
        left_eye_pred = np.argmax(predictions, axis=1)
